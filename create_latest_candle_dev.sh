...
# netplan* 
apt remove firefox* galculator* geany* thonny* wolfram-engine* oracle-java* scratch* libreoffice* cloud* --purge -y 1> /dev/null
...
#    set +e
	...
	# For Thread / Matter 
	apt install -y --no-install-recommends ipset tio bluez pi-bluetooth 
    ...
    #apt install -y --no-install-recommends hostapd "$reinstall" #--print-uris 
...
systemctl enable candle_emergency_hotspot.service
...
systemctl enable candle_every_few_seconds.service
