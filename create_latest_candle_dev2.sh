...
#    set +e
	...
	apt install -y --no-install-recommends dns-root-data dnsmasq libbluetooth3 libndp0 libnetfilter-conntrack3 libnfnetlink0 iptables dbus-launch
	...
	if [ ! -f /usr/bin/python3.11 ]; then
		...
		apt install build-essential zlib1g-dev libncurses-dev libgdbm-dev libnss3-dev libsqlite3-dev libssl-dev libreadline-dev libffi-dev wget -no-install-recommends -y
    ...
    apt install -y --no-install-recommends xserver-xorg-core x11-xserver-utils xauth xinit xinput xserver-xorg-input-evdev feh edid-decode unclutter dbus-x11 xdotool xfonts-base fonts-noto-color-emoji fonts-symbola
...
# pipewire uses a lot of memory, so not using it right now
#apt install -y pipewire libspa-0.2-bluetooth
#raspi-config nonint do_audioconf 2
...
if [ -f /etc/resolvconf/resolv.conf.d/head ]; then
	echo "#generated by resolvconf" > /etc/resolvconf/resolv.conf.d/head
fi
